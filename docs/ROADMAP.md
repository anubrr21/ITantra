...
- [ ] **Phase 2 — VAD + STT bring-up (Hindi + English).** Swap `EnergyVoiceActivityDetector`
      for WebRTC VAD (real pause/stop detection, per spec). Wire buffered utterances into
      Vosk (small, already Android-ready, ~50MB/language) as a fast baseline STT. Use
      Android's built-in TTS engine to close the loop end-to-end (text → speech) before
      touching any custom model.
